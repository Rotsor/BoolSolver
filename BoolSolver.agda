...
boolOpSem : ∀ (op : BinOp) → Bool → Bool → Bool
boolOpSem And = Data.Bool._∧_
boolOpSem Or = Data.Bool._∨_

const-bin-sem : ∀ op a b → const (boolOpSem op a b) ≈ bin op (const a) (const b)
const-bin-sem And true b = let open Solver in solve 1 (λ b → b ⊜ :⊤ :∧ b) refl _
const-bin-sem And false b = let open Solver in solve 1 (λ b → :⊥ ⊜ :⊥ :∧ b) refl _
const-bin-sem Or true b = let open Solver in solve 1 (λ b → :⊤ ⊜ :⊤ :∨ b) refl _
const-bin-sem Or false b = let open Solver in solve 1 (λ b → b ⊜ :⊥ :∨ b) refl _
...
... | a' , a-proof | b' , b-proof = (boolOpSem op a' b') , (trans (bin-cong op a-proof b-proof) (sym (const-bin-sem op a' b')))
